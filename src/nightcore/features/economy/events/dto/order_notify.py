"""DTO for coins shop order notification event."""
...
@dataclass
class CoinsShopOrderNotifyDTO(BaseEventDTO):
    guild: Guild
    event_type: str
    user_id: int
    moderator_id: int
    ...
    def build_log_embed(self, bot: "Nightcore") -> Embed:
        ...
        return (
            Embed(
                title="Покупка в магазине коинов",
            ...
            .add_field(
                ...
                value=f"**{self.user_balance_before}**",
                inline=False,
            )
            .add_field(
                name="Баланс после покупки",
                value=f"**{self.user_balance_after}**",
            )
            .add_field(
                name="Модератор",
