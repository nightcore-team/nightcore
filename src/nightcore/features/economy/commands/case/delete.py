...
@case_group.command(name="delete", description="Удалить кейс")  # type: ignore
...
async def delete_case(
    ...
    try:
        ...
        outcome = "case_delete_error"

        logger.exception(
            "[case/delete] Error delete case in guild %s: %s",
            guild.id,
            e,
        )
    ...
    logger.info(
        "[command] - invoked guild=%s user=%s case_id=%s",
        guild.id,
        interaction.user.id,
        case_id,
    )
