...
class UserRoleRequestBannedEvent(Cog):
    def __init__(self, bot: Nightcore) -> None:
        self.bot = bot

    @Cog.listener()
    async def on_user_role_request_banned(
        self,
        *,
        ...
        logger.info(
            "[event] on_user_role_request_banned - %s: Guild: %s, Member: %s, Reason: %s, Duration: %s",  # noqa: E501
            data.category,
        ...
        async with self.bot.uow.start() as session:
            try:
                ...
                logger.exception(
                    "[event] on_user_role_request_banned - %s: Failed to create punish record: %s",  # noqa: E501
                    data.category,
                    e,
                )
                return
    ...
    async def on_user_unrole_request_banned(
        ...
        """Handle user unrole_request_banned events."""
        logger.info(
            "[event] on_user_unrole_request_banned - %s: Guild: %s, Member: %s, Reason: %s",  # noqa: E501
            data.category,
            data.guild_id,
            data.user_id,
            data.reason,
        ...
        async with self.bot.uow.start() as session:
            if by_command:
                ...
                if temp:
                    ...
                    logger.error(
                        "[event] on_user_unrole_request_banned - %s: No active temporary punishment found for user %s in guild %s",  # noqa: E501
                        data.category,
                        data.user_id,
                        data.guild_id,
                    )
...
async def setup(bot: Nightcore):
    """Setup the UserRoleRequestBannedEvent cog."""
    await bot.add_cog(UserRoleRequestBannedEvent(bot))
